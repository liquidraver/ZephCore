...
// ZEPHCORE: deadline-driven maintenance and the adaptive contention window.
...
uint32_t Mesh::msUntilNextMaintenance() {
  return maintenanceSooner(Dispatcher::msUntilNextMaintenance(),
                           _contention.msUntilNextTick((uint32_t)_ms->getMillis()));
}

// A duplicate of a flood we are about to retransmit was heard: defer our copy
// by one backoff_multiplier x airtime window (per dupe, from NOW), within the
// per-packet reactive headroom.
void Mesh::extendPendingRetransmit(uint32_t hash32) {
  ...
  for (int i = 0; i < total; i++) {
    ...
    if (pkt && pkt->isRouteFlood() && ContentionTracker::computePacketHash32(pkt) == hash32) {
...
// Adaptive flood jitter: 5 x airtime x EMA factor, capped at 6 airtimes and
// 2000 ms absolute; +20 ms floor so downstream nodes are back in RX first.
...
// Direct: jitter around upstream's 0.3 factor, per-packet in [0.25, 0.40]; +20 ms floor.
...
DispatcherAction Mesh::onRecvPacket(Packet* pkt) {
  if (pkt->isRouteDirect() && pkt->getPayloadType() == PAYLOAD_TYPE_TRACE) {
    if (pkt->path_len < MAX_PATH_SIZE && pkt->payload_len >= 9) {  // ZEPHCORE: header must be present
      uint8_t i = 0;
      uint32_t trace_tag;
  ...
  // ZEPHCORE: record dupes for contention tracking + reactive backoff; on nodes
  // that don't relay, track first hearings so the EMA still reflects contention.
  if (pkt->isRouteFlood()) {
  ...
  switch (pkt->getPayloadType()) {
          ...
          for (int j = 0; j < num; j++) {
            ...
            if (len > 0) {  // success!
              if (pkt->getPayloadType() == PAYLOAD_TYPE_PATH) {
                ...
                if (k + hash_size*hash_count + 1 > len) {  // ZEPHCORE: inner path must fit the decrypted data
    ...
    case PAYLOAD_TYPE_MULTIPART:
      if (pkt->payload_len > 2) {
        ...
        (void)remaining;  // ZEPHCORE: -Wunused-variable
...
DispatcherAction Mesh::routeRecvPacket(Packet* packet) {
  ...
  if (packet->isRouteFlood() && !packet->isMarkedDoNotRetransmit()
    ...
    // ZEPHCORE: track our retransmit so heard dupes can be counted.
    _contention.trackRetransmit(ContentionTracker::computePacketHash32(packet), (uint32_t)_ms->getMillis());
...
Packet* Mesh::createAdvert(const LocalIdentity& id, const uint8_t* app_data, size_t app_data_len) {
  ...
  if (app_data_len > 0) {  // ZEPHCORE: app_data may be NULL
...
Packet* Mesh::createMultiAck(const uint8_t* ack, uint8_t len, uint8_t remaining) {
  if (1 + (size_t)len > MAX_PACKET_PAYLOAD) return NULL;  // ZEPHCORE: bound
...
void Mesh::sendFlood(Packet* packet, uint32_t delay_millis, uint8_t path_hash_size) {
  if (packet->getPayloadType() == PAYLOAD_TYPE_TRACE) {
    ...
    releasePacket(packet);  // ZEPHCORE: don't leak the pool slot
    return;
  }
  ...
  sendPacket(packet, pri, delay_millis + getInitialFloodJitter(packet));  // ZEPHCORE: initial-flood jitter
}
...
void Mesh::sendDirect(Packet* packet, const uint8_t* path, uint8_t path_len, uint32_t delay_millis) {
  ...
  if (packet->getPayloadType() == PAYLOAD_TYPE_TRACE) {   // TRACE packets are different
    // ZEPHCORE: a crafted CMD_SEND_TRACE_PATH can set a path hash size the
    // caller's hop-count check doesn't byte-bound; keep the append in payload[].
