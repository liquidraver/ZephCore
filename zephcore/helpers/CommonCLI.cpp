...
#ifndef ZEPHCORE_COMPANION  /* no region map on a companion */
	} else if (_region_map && memcmp(command, "region", 6) == 0) {
		handleRegionCmdCopy(sender_timestamp, command, reply);
#endif
...
/* Radio keys, first, as upstream's CommonRadioPrefs::handleCommand. The
 * branches keep their relative order: several match on a prefix (radio vs
 * radio.rxgain, freq vs freqerr). */
bool CommonCLI::handleRadioCmd(uint32_t sender_timestamp, const char* command, char* reply) {
...
void CommonCLI::handleGetCmd(uint32_t sender_timestamp, const char* command, char* reply) {
	 ...
	 * field after it and invalidate existing prefs files. */
		...
		*reply = 0;
		...
		loopWakeFormat(reply, replyCap(sender_timestamp));
#endif
...
void CommonCLI::handleSetCmd(uint32_t sender_timestamp, const char* command, char* reply) {
	const char* config = &command[4];
	if (memcmp(config, "leds.radio ", 11) == 0) {
...
#ifndef ZEPHCORE_COMPANION  /* the companion has no region map */

/* ---- region CLI (upstream CommonCLI::handleRegionCmd) ---- */

/* parseTextParts needs a writable command; ours arrives const. The copy lives
 * here, not in handleCommand, so it costs stack only for a region command. */
__attribute__((noinline)) void CommonCLI::handleRegionCmdCopy(uint32_t sender_timestamp, const char* command, char* reply) {
	char buf[CLI_REPLY_SIZE];
	strncpy(buf, command, sizeof(buf) - 1);
	buf[sizeof(buf) - 1] = 0;
	handleRegionCmd(sender_timestamp, buf, reply);
}
...
static bool processRegionDefSegment(RegionMap* map, char* tok, RegionEntry** cursor, char* reply, int cap) {
	char* jump = splitNameJump(tok);
	char* name = skipSpaces(tok);
	if (*name == '\0') { snprintf(reply, cap, "Err - empty name"); return false; }
	if (jump && *jump == '\0') { snprintf(reply, cap, "Err - empty jump"); return false; }

	RegionEntry* r = map->putRegion(name, (*cursor)->id);
	if (r == NULL) { snprintf(reply, cap, "Err - put failed: %s", name); return false; }
	r->flags = 0;

	if (jump) {
		RegionEntry* j = map->findByNamePrefix(jump);
		if (j == NULL) { snprintf(reply, cap, "Err - unknown jump: %s", jump); return false; }
		*cursor = j;
	} else {
		*cursor = r;
	}
...
void CommonCLI::handleRegionCmd(uint32_t sender_timestamp, char* command, char* reply) {
	reply[0] = 0;
	// ZEPHCORE: upstream writes up to 160 bytes; a remote reply behind a reflected
	// "xx|" prefix has only CLI_REMOTE_REPLY_SIZE - 3, so a large region map
	// overran the caller's buffer by 2 bytes.
	const int cap = replyCap(sender_timestamp) < 160 ? (int)replyCap(sender_timestamp) : 160;
	...
	if (strncmp(cmd, "region def", 10) == 0 && (cmd[10] == ' ' || cmd[10] == '\0')) {
		...
		rtrimSpaces(payload);
		if (*payload == '\0') { snprintf(reply, cap, "Err - empty def"); return; }

		RegionEntry* cursor = &_region_map->getWildcard();
		for (char* tok; (tok = takeToken(&payload)) != nullptr; ) {
			if (!processRegionDefSegment(_region_map, tok, &cursor, reply, cap)) return;
		}
		_region_map->exportTo(reply, cap);
		return;
	}
	...
	if (n == 1) {
		...
		int len = _region_map->exportNamesTo(reply, cap, mask, invert);
		if (len == 0) {
			strcpy(reply, "-none-");
		}
	} else {
