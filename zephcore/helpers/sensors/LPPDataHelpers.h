...
public:
  static uint8_t getDataSize(uint8_t type) {
    switch (type) {
      ...
      // ZEPHCORE: LPP_RELATIVE_HUMIDITY is 1 byte (see readRelativeHumidity()
      // below and the CayenneLPP spec); listed here it misaligned every field
      // after a humidity reading. It falls through to the 1-byte default.
      case LPP_ALTITUDE:
      case LPP_VOLTAGE:
      case LPP_CURRENT:
      case LPP_DIRECTION:
