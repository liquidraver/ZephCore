...
public:
  ...
  size_t writeFrame(const uint8_t src[], size_t len) override {
    ...
    bool anyConnected = false;  // ZEPHCORE
    for(auto iface : _interfaces){
      // ZEPHCORE: connected ones only. An enabled but idle BLE/WiFi interface
      // returns 0, which failed every write, and the caller retries lossless
      // replies on 0.
      if(iface.instance && iface.instance->isEnabled() && iface.instance->isConnected()){
        anyConnected = true;  // ZEPHCORE
    ...
    return (allSuccessful && anyConnected) ? len : 0;  // ZEPHCORE: 0 when nobody is listening
  }
