...
int RegionMap::getTransportKeysFor(const RegionEntry& src, TransportKey dest[], int max_num) {
  ...
  if (src.name[0] == '$') {   // private region
    ...
    tmp[0] = '#';
    memcpy(&tmp[1], src.name, sizeof(src.name) - 1);  // ZEPHCORE: bounded, the name
    tmp[sizeof(src.name)] = 0;                        // may fill all 31 bytes
...
// ZEPHCORE: whole lines only. Upstream's BufStream cuts the last line mid-name
// when the map overflows the buffer; a cut name pasted back into `region load`
// would define a wrong region. A line is written only if it fits entirely.
class LineBufStream : public Stream {
public:
  LineBufStream(char *buf, size_t max_len) : _buf(buf), _max_len(max_len), _pos(0), _line_len(0) {
    if (_max_len > 0) _buf[0] = 0;
  }
  size_t write(uint8_t c) override {
    if (_line_len < sizeof(_line)) _line[_line_len++] = c;
    if (c == '\n') {
      if (_pos + _line_len < _max_len) {
        memcpy(&_buf[_pos], _line, _line_len);
        _pos += _line_len;
        _buf[_pos] = 0;
      }
      _line_len = 0;
...
size_t RegionMap::exportTo(char *dest, size_t max_len) const {
  ...
  LineBufStream bs(dest, max_len);  // ZEPHCORE: was BufStream
