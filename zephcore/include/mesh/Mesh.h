...
#include <mesh/ContentionTracker.h>  // ZEPHCORE: adaptive contention window
...
public:
  ...
  // ZEPHCORE: wasSeen() is a pure query; every insert is an explicit markSeen().
};
...
class Mesh : public Dispatcher {
  RNG* _rng;         // ZEPHCORE: declared in constructor-init order (-Wreorder);
  RTCClock* _rtc;    // upstream declares _rtc first
  MeshTables* _tables;
...
protected:
  // ZEPHCORE: adaptive contention window — dupe tracking, reactive backoff,
  // adaptive retransmit jitter (docs: ADAPTIVE contention, LLD 02).
  ...
  // Track heard floods we don't forward, so the EMA warms on non-relaying nodes.
  virtual bool passivelyTrackFloods() const { return false; }
  // Added to the caller's delay on every sendFlood(); companions spread their
  // originated floods, repeaters keep 0.
  virtual uint32_t getInitialFloodJitter(const Packet* packet) { return 0; }
...
public:
  ...
  // ZEPHCORE: deadline-driven maintenance (see Dispatcher).
  void maintenanceLoop();
  uint32_t msUntilNextMaintenance() override;

  // ZEPHCORE: the UI asks for per-packet dupe counts ("heard a repeat?").
