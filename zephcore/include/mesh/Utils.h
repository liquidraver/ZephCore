...
public:
  ...
  */
  ...
  // ZEPHCORE: crypto hygiene helpers.
  // Constant-time equality: no early exit, so timing is independent of the
  // input (MAC / password / secret compares). The volatile accumulator survives
  // -Os LTO (disassembly-verified on Cortex-M4). Pattern from rweather/arduinolibs.
  static bool constantTimeEqual(const void* a, const void* b, size_t n);
  // Zero a buffer the compiler cannot elide as a dead store (key material, seeds,
  // shared secrets after last use).
  static void secureZeroize(void* buf, size_t n);
  // An entropy source, KDF or other primitive could not produce a safe result:
  // log and cold-reboot rather than risk weak keys or bypassed auth.
