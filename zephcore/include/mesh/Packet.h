...
public:
  ...
  uint8_t header;
  // ZEPHCORE: path_len stays uint8_t (the wire byte); upstream widened it to
  // uint16_t, which changes the wrap behaviour of the TRACE path arithmetic.
  ...
  // ZEPHCORE: src_len bounds the source. Trusted callers copying from an
  // internal MAX_PATH_SIZE buffer pass MAX_PATH_SIZE; callers handling
  // untrusted input (phone frames, anon-request payloads) pass the real
  // remaining length, so a malformed path_len cannot read out of bounds.
  static uint8_t copyPath(uint8_t* dest, const uint8_t* src, size_t src_len, uint8_t path_len);  // returns path_len, 0 if rejected
  static size_t writePath(uint8_t* dest, const uint8_t* src, size_t src_len, uint8_t path_len);  // returns byte length written
  // ZEPHCORE: upstream's signatures, for trusted sources (internal MAX_PATH_SIZE buffers).
  static uint8_t copyPath(uint8_t* dest, const uint8_t* src, uint8_t path_len) { return copyPath(dest, src, MAX_PATH_SIZE, path_len); }
  static size_t writePath(uint8_t* dest, const uint8_t* src, uint8_t path_len) { return writePath(dest, src, MAX_PATH_SIZE, path_len); }
